...
def populate():
    file_path = os.path.join(os.path.dirname(os.path.abspath(__file__)), '../../sample_users_data.json')
    enc = chardet.detect(open(file_path, "rb").read())['encoding']
    with open(file_path, "r", encoding=enc) as file:
        file_data = json.load(file)
        if isinstance(file_data, list):
            users_collection.insert_many(file_data)
        else:
            users_collection.insert_one(file_data)
...
_Paths = {
    ...
    "friends": "layouts/friends.asp",
    "messages": "layouts/messages.asp",
    "explore": "layouts/explore.asp",
    "profile": "layouts/profile.asp",
    "notifications": "layouts/notifications.asp",
    "calls": "layouts/calls.asp",
    "newpost": "layouts/newpost.asp",
    "settings": "layouts/settings.asp",
    "createpost": "v1/intn/createpost.asp",
    "foryouposts": "v1/posts.asp",
    "stories": "v1/stories.asp",
    "videoposts": "v1/posts.asp",
    "marketposts": "v1/posts.asp",
    "postview": "v1/intn/postview.asp",
    "relatedposts": "v1/int_posts/relatedposts.asp",
    "foryoufriends": "v1/int_friends/foryoufriends.asp",
    "storyview": "v1/intn/storyview.asp",
    "profileview": "v1/intn/profileview.asp",
    "chatview": "v1/intn/chatview.asp",
    "active_users": "v1/active_users.asp",
    "user_chats": "v1/user_chats.asp",
...
#            user_id = "z80g82dk1qtt3nyvz2n9rv9kocbts7l5h6it"
            try:
                ...
                recommended_posts = recommender.recommend_posts(user_id, 16)
            ...
            try:
                # Create an instance of HybridRecommender and call recommend_stories
                recommender = HybridRecommender()
                recommended_stories = recommender.recommend_stories(user_id, 10)
                ...
                data = [
                    {
                        ...
                        "user": {
                            "user_id": story["user"].get("user_id", ""),
                            "username": story["user"].get("username", ""),
                            "profile": story["user"].get("profile", ""),
                            "is_verified": story["user"].get("is_verified")
                        },
                ...
                # Debugging output
                if not data:
                    debug_logger.info(f"No data returned by recommend_stories for user_id: {user_id}")

            except Exception as e:
                debug_logger.error(f"Error in recommend_stories: {e}")
                data = []  # Fallback to empty data

        elif page == "videoposts":
            data = get_videoposts(user_id)

        elif page == "marketposts":
            data = get_marketposts(user_id)

        elif page == "relatedposts":
            data = get_related_posts()

        elif page == "active_users":
            data = get_active_users()

        elif page == "foryoufollowers":
            recommender = HybridRecommender()
            data = recommender.recommend_followers(user_id, 12)
            
        elif page == "user_chats":
            data = get_user_chats(user_id)
                
        elif page == "postview":
            post = get_single_post(_data['view_id'])
            # Transform data into the format expected by the template
            data = [
                {
                    "post_id": post["post_id"],
                    ...
                    "post_tags": post.get("post_tags", [])[:2],
                    "created_at": post.get("created_at", ""),
                    "post_url": post.get("post_url", ""),
                    "post_type": post.get("post_type", ""),
                    "likes": post.get("likes", []),
            ...
            story = get_single_story(_data['view_id'])
            data = [
                {
                    "story_id": story["story_id"],
                    "caption": story.get("caption", ""),
                    "user": {
                        "user_id": story["user"].get("user_id", ""),
                        "username": story["user"].get("username", ""),
                        "first_name": story["user"].get("first_name", ""),
                        "last_name": story["user"].get("last_name", ""),
                        "profile": story["user"].get("profile", ""),
                        "is_verified": story["user"].get("is_verified")
                    },
                    "story_tags": story.get("story_tags", [])[:3],
                    "created_at": story.get("created_at", ""),
                    "story_url": story.get("story_url", ""),
            ...
            data = get_single_post(_data['view_id'])
